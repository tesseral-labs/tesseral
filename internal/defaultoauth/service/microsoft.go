...
func (s *Service) microsoftOAuthCallback(w http.ResponseWriter, r *http.Request) error {
	ctx := r.Context()
	...
	vaultDomain, err := s.Store.GetVaultDomainByMicrosoftOAuthState(ctx, state)
	if err != nil {
		return connect.NewError(connect.CodeInternal, fmt.Errorf("get vault domain: %w", err))
	}

	redirectURL, err := url.Parse(fmt.Sprintf("https://%s/microsoft-oauth-callback", vaultDomain))
	if err != nil {
