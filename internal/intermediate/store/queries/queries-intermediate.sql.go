...
func (q *Queries) GetOrganizationPrimaryOIDCConnection(ctx context.Context, organizationID uuid.UUID) (OidcConnection, error) {
	row := q.db.QueryRow(ctx, getOrganizationPrimaryOIDCConnection, organizationID)
...
type ListOIDCOrganizationsParams struct {
	ProjectID uuid.UUID
	Domain    string
}

func (q *Queries) ListOIDCOrganizations(ctx context.Context, arg ListOIDCOrganizationsParams) ([]Organization, error) {
	rows, err := q.db.Query(ctx, listOIDCOrganizations, arg.ProjectID, arg.Domain)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
