...
export function AssignUserRolesButton({ userId }: { userId: string }) {
  const { data: listUserRoleAssignmentsResponse, refetch } = useQuery(
    listUserRoleAssignments,
    {
      userId,
    },
  ...
  }, [form, listUserRoleAssignmentsResponse]);
  ...
  async function handleSubmit(data: z.infer<typeof schema>) {
    ...
    const createAssignments = newRoles.filter(
      (role) => !oldRoles?.includes(role),
    );
    const deleteAssignments = oldRoles.filter(
