...
import { LoginPage, LoginPageMap } from '@/lib/login-pages';
...
const ChooseProjectPage: FC = () => {
  ...
  const deriveNextPage = (organization: Organization): string | undefined => {
    if (organization.logInWithPassword) {
      if (organization.userHasPassword) {
        return LoginPageMap[LoginPage.VerifyPassword];
      }

      return LoginPageMap[LoginPage.RegisterPassword];
    } else if (
      organization.userHasAuthenticatorApp &&
      !organization.userHasPasskey
    ) {
      return LoginPageMap[LoginPage.VerifyAuthenticatorApp];
    } else if (
      organization.userHasPasskey &&
      !organization.userHasAuthenticatorApp
    ) {
      return LoginPageMap[LoginPage.VerifyPasskey];
    } else if (organization.requireMfa) {
      // this is the case where the organization has multiple secondary factors and requires mfa
      return LoginPageMap[LoginPage.ChooseAdditionalFactor];
    }
  };
  ...
  useEffect(() => {
    if (listOrganizationsResponse?.organizations?.length === 0) {
      navigate(LoginPageMap[LoginPage.CreateProject]);
    }
      ...
      <Card className="w-full max-w-sm">
        ...
        <CardContent className="flex flex-col items-center justify-center w-full">
          ...
          <div className="w-full">
            ...
            <Button
              className="w-full"
              onClick={() => navigate(LoginPageMap[LoginPage.CreateProject])}
            >
              Create a Project
            </Button>
          </div>
