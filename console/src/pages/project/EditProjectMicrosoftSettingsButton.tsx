  ...
  useEffect(() => {
    if (getProjectResponse?.project) {
      form.reset({
        ...
        microsoftOAuthClientId: getProjectResponse.project.microsoftOauthClientId,
        microsoftOAuthClientSecret: '',
      });
    }
  }, [getProjectResponse]);
    ...
    if (!values.logInWithMicrosoft) {
      if (
        ...
        form.setError('logInWithMicrosoft', {
          message:
            'At least one of Log in with Email, Log in with Microsoft, or Log in with Google must be enabled.',
        });
        return;
      }
    }
